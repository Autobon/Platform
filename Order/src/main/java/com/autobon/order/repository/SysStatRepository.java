...
public interface SysStatRepository extends JpaRepository<SysStat, Integer> {
     ...
     //统计某个日期段内
     @Query("SELECT s from SysStat s where s.statTime between ?1 and ?2 and s.statType=1")
     List<SysStat> findBetweenOfDay(Date start,Date end);

     //统计某个月份段内
     @Query("SELECT s from SysStat s where s.statTime between ?1 and ?2 and s.statType=2")
