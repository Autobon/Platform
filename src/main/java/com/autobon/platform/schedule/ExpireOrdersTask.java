...
public class ExpireOrdersTask {
  ...
  //  @Scheduled(fixedRate = 3600*1000, initialDelay = 300*1000)
    public void expireOrders() {
        ...
        Date finishBefore = Date.from(LocalDateTime.now().minusDays(3).atZone(ZoneId.systemDefault()).toInstant());
        int totalPages = 0;
        int pageNo = 1;
        long orderCount = 0;
        do {
            Page<Order> page = orderService.findExpired(signInBefore, finishBefore, pageNo++, 20);
