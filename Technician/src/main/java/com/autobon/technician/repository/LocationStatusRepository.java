...
public interface LocationStatusRepository extends JpaRepository<LocationStatus,Integer> {
    ...
    @Query(value = "SELECT " +
            ...
            " where t.status = 2 and round(6378.138*2*asin(sqrt(pow(sin( (ls.lat*pi()/180-?1*pi()/180)/2),2)+cos(ls.lat*pi()/180)*cos(?1 *pi()/180)* pow(sin( (ls.lng*pi()/180-?2*pi()/180)/2),2)))*1000) <?3"
            ,nativeQuery = true)
